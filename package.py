...
class Environment:
    ...
    def temp_dir() -> str:
        ...
        if temp is None:
            logger.error('Required TEMP env variable was not set')
            exit(1)
        return temp
...
class WindowsPackaging:
    def __init__(self, storage: Storage, env: Environment) -> None:
        ...
        self.__env = env
        self.__p12 = str(Path(env.temp_dir()).joinpath('certificate.p12').absolute())
    ...
    def import_signing_certificates(self) -> bool:
        ...
        p12 = self.__p12
        os.environ.setdefault('WIN_CSC_LINK', p12)
